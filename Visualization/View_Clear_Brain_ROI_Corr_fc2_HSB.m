function pairsOut=View_Clear_Brain_ROI_Corr_fc2_HSB(Anat,IM,M,dataOut,fc,Pmax,params)
%
...
if ~isfield(params, 'cMapN'), 
cMapN=hsv(2800);cMapN=cMapN(1401:2400,:);
else cMapN=params.cMapP;
end


if ~isfield(params, 'cMapP'), 
cMapP=cat(1,summer(500),flip(autumn(500),1));
else cMapP=params.cMapP;
end
...
figure('Color','w');fcBx_PercFC_Scat2_HSB(pairsOut{ts},IM); %perc f>0
end
disp(['Drawing ROIs for ',IM.Nets{NNsig(j,1)},...
    ' and ',IM.Nets{NNsig(j,2)}])
Draw_ROIs_Through_Cortex_3_Views_HSB(Anat,roi,Conn,params);
%end
