...
kdenth = stats.kdenth;
params = stats.params;
binarize = params.binary;
%% Set parameters
[Nroi,Nkden]=size(clrs);
metrics.modularity=zeros(Nkden,1);
   ...
   if strcmp(params.type,'mst')
    ...
    if domod==1
        ...
        if isempty(Ugroups)
            ...
            tr=trace(e0);
            e2=sum((sum(e0)).^2);
            metrics.modularity(j)=tr-e2;
        end
    end
    
    %         disp('Calculate Nedges');
    metrics.Nedges(j)=sum(rmat(UDidx)~=0);
    %         disp('Calculate kave');
    metrics.kave(j)=(metrics.Nedges(j)*2)/Nroi;
    ...
    [~,D]=eig(M);
    D=abs(diag(D));
    metrics.Nc(j)=sum(D(:)<1e-4); % Ncomponents
    D=zeros(Nroi);
    n=1;nPATH=rmat;L=(nPATH~=0);
    while find(L,1), D=D+n.*L;n=n+1;nPATH=nPATH*rmat;L=(nPATH~=0).*(D==0);end
    R=single(D~=0);
    metrics.NnBc(j)=max(sum(R))/Nroi; % Nn in biggest component
